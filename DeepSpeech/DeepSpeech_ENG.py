...
from util.feeding_ENG import DataSet, ModelFeeder
from util.gpu import get_available_gpus
from util.shared_lib import check_cupti
from util.text_ENG import sparse_tensor_value_to_texts, wer, levenshtein, Alphabet, ndarray_to_text, cer
...
tf.app.flags.DEFINE_string  ('lm_binary_path',       'data/smoke_test/lm.binary', 'path to the language model binary file created with KenLM')
tf.app.flags.DEFINE_string  ('lm_trie_path',         'data/smoke_test/trie', 'path to the language model trie file created with native_client/generate_trie')
...
def calculate_report(results_tuple):
    ...
    with open('test_set_results.csv', 'a') as csvfile:
        wt = csv.writer(csvfile, quotechar='|', quoting=csv.QUOTE_MINIMAL)
        for label, decoding, distance, loss in items:
            sample_wer = cer(label, decoding)
            ...
            wt.writerow([label, decoding, distance, loss, sample_wer])
...
def do_single_file_inference(input_file_path):
    with tf.Session(config=session_config) as session:
        ...
        logits = session.run(outputs['logits'], feed_dict = {
                    inputs['input']: [mfcc],
                    inputs['input_lengths']: [len(mfcc)],
        })
        ...
        for i in range(0, len(logits)):
            for j in range(0, len(logits[i])):
                tmp_list = []
                for k in sorted(logits[i][j], reverse=True)[:10] :
                        tmp_list.append(str(np.where(logits[i][j] == k)[0][0]))
                rank.append(tmp_list)
                prb.append(max(logits[i][j]))
                for k in range(0, len(logits[i][j])):
                    if max(logits[i][j]) == logits[i][j][k]:
                        org.append(k)

        text = ndarray_to_text(output[0][0], alphabet)

        wfile = wave.open(input_file_path, 'r')
        print('파일 재생 시간(초) : ', (1.0 * wfile.getnframes ()) / wfile.getframerate (), '\n')
        print('총 frame 수 : ', len(logits), '\n')
        '''
        print('첫 frame의 mfcc 값', mfcc[0], '\n')
        print('첫번째 frame의 results vector : ', logits[0][0], '\n')
        print('각 frame의 max value : ', prb, '\n')
        print('각 frame의 max value를 갖는 index: ', org, '\n')
        lst = ''
        for i in org :
            if i == 69:
                lst += '*'
            ...
            if i >= 41:
                i -= 1
            lst += alphabet.string_from_label(i)
        print('max value index에 해당하는 character로 변환: ', lst.encode('utf-8'), '\n')
        '''
        rank_list = []
        for i in rank :
            write = []
            for j in i :
                ...
                if id >= 41:
                    id -= 1
                write.append("  "+alphabet.string_from_label(id))
            rank_list.append(write)

        e = int(time.time() - start_time)
        print("------------------------------------------")
        print("frame\trank1\trank2\trank3")
        print("------------------------------------------")
        for i in range(0, len(logits)) :
            print(str(i) + "\t" + rank_list[i][0].encode('utf-8') + "\t" + rank_list[i][1].encode('utf-8') + "\t" + rank_list[i][2].encode('utf-8'))
        print("------------------------------------------")
        print('duration : {:02d}:{:02d}:{:02d}'.format(e // 3600, (e % 3600 // 60), e % 60))
        print("------------------------------------------")
        print('인식 결과 : ', text.encode('utf8'))
