        ...
        with codecs.open(config_file, 'r', 'utf-8') as fin:
            for line in fin:
                if line[0:2] == '\\#':
                    ...
                    continue
                self._label_to_str.append(line[:-1]) # remove the line ending
...
def text_to_char_array(original, alphabet):
    ...
    for char in original:
      if ord(char) == 32:
        sequence.append(0)
        continue
      cho, jung, jong = char_to_code(char)
      if not cho == 12:
        sequence.append(cho)
      sequence.append(jung)
      if not jong == 41:
          sequence.append(jong)
    return np.asarray([c for c in sequence])
...
def call_code_to_char(buf, alphabet):
  if len(buf) == 0:
    return ''
  if buf[0] > 41:  # Jongs only, I believe it is not possible.
    return alphabet.string_from_label(buf[0])
  if buf[0] >= 20 and buf[0] <= 41:
    buf.insert(0, 12)
  if buf[0] < 20 and len(buf) == 1:  # Chos only
    return alphabet.string_from_label(buf[0])
  if buf[0] < 20 and buf[1] > 41:  # Chos+Jongs, I believe it is not possbile, too.
    return alphabet.string_from_label(buf[0])+alphabet.string_from_label(buf[1])
  if len(buf) == 2:
    return unichr(((buf[0]-1)*21 + (buf[1]-20))*28 + 0xAC00)
  elif len(buf) == 3:
    return unichr(((buf[0]-1)*21 + (buf[1]-20))*28 + buf[2] - 41 + 0xAC00)

def sparse_tuple_to_texts(tuple, alphabet):
    indices = tuple[0]
    values = tuple[1]
    results = [[] for _ in range(tuple[2][0])]
    for i in range(len(indices)):
        index = indices[i][0]
	results[index].append(values[i])
    for i in range(len(results)):
	results[i] = ndarray_to_text(results[i], alphabet)
    return results
...
def ndarray_to_text(value, alphabet):
    ...
    for code in value:
        if code == 0:
            string.append(call_code_to_char(buf, alphabet))
            buf = []
            string.append(" ")
            continue
        if code < 20:
            ...
            buf.append(code)
            prev = code
        elif code >= 20 and code < 41:
            if prev >= 20 or prev < 0:
                string.append(call_code_to_char(buf, alphabet))
                buf = []
            buf.append(code)
            prev = code
...
def cer(original, result):
    ...
    res = []
    res += result
    return levenshtein(org, res) / float(len(org))
