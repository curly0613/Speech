...
def audioToInputVector(audio, fs, numcep, numcontext):
    ...
    # Get mfcc coefficients and calculate delta
    features = mfcc(audio, samplerate=fs, numcep=numcep)
    #d_mfcc_feat = delta(features, 2)
    #features = np.hstack([features, d_mfcc_feat])
    #a_mfcc_feat = delta(d_mfcc_feat, 2)
    #features = np.hstack([features, d_mfcc_feat, a_mfcc_feat])
